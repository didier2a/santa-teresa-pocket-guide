...
import {EventEmitter} from '../vendor/liveavatar/events-browser.mjs';

const read=path=>readFile(new URL(`../${path}`,import.meta.url),'utf8');
const [endpoint,controller,runtime,css,html,sw,eventsShim,util,envText,vercelText]=await Promise.all([
  read('api/liveavatar-session.js'),read('js/pg23/avatar/liveavatar-realtime-controller.js'),read('js/pg23/bootstrap/living-companion-runtime.js'),read('pocketguide-v23.css'),read('pocketguide-v23.html'),read('service-worker.js'),read('vendor/liveavatar/events-browser.mjs'),read('api/_util.js'),read('.env.example'),read('vercel.json')
]);
...
test('le mode LiveAvatar Realtime reste explicitement opt-in sur PocketGuide 2.3.2',()=>{
  ...
  assert.match(runtime,/liveAvatarRealtimeRequested\(\)\?liveAvatarRealtimeController:avatarEngineController/);assert.match(html,/data-pg-version="2\.3\.2"/);assert.match(html,/app\.js\?v=2\.3\.2\.14/);
});
...
test('OpenAI Realtime marin pilote la vidéo LiveAvatar verticale et son labial',()=>{
  ...
  assert.match(controller,/@heygen\/liveavatar-web-sdk@0\.0\.18\/dist\/index\.esm\.js/);assert.match(html,/"events":"\.\/vendor\/liveavatar\/events-browser\.mjs"/);assert.match(eventsShim,/export class EventEmitter/);assert.match(controller,/new sdk\.LiveAvatarSession/);assert.match(controller,/session\.attach\(video\)/);assert.match(controller,/voiceChat/);assert.match(controller,/Pocket-Guide-LiveAvatar-1080x1920\.jpg/);assert.match(css,/\.liveavatar-realtime-video\{[^}]*object-fit:cover/);
});

test('le polyfill navigateur fournit les événements requis par le SDK LiveAvatar',()=>{
  const emitter=new EventEmitter(),values=[];const persistent=value=>values.push(`on:${value}`);emitter.on('turn',persistent);emitter.once('turn',value=>values.push(`once:${value}`));assert.equal(emitter.emit('turn','a'),true);assert.equal(emitter.emit('turn','b'),true);emitter.off('turn',persistent);assert.deepEqual(values,['on:a','once:a','on:b']);assert.equal(emitter.listenerCount('turn'),0);
});
