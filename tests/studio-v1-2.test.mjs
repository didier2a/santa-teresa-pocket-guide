...
test('V1.3: Studio expose prompt, import, validation, partage et export',async()=>{const html=await read('studio.html'),js=await read('js/studio-v1-3.js');for(const token of ['Décrivez votre voyage','Importer RoutePack','Validation et publication','Créer le lien','Télécharger JSON','Parcours publiés'])assert.match(html,new RegExp(token));assert.match(js,/validateRoutePack/);assert.match(js,/packShareUrl/);assert.match(js,/routeShareUrl/);assert.match(js,/routepack\.json/);assert.match(js,/compilePrompt/)});
