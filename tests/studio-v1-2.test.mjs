...
test('V1.2: Bonifacio est un second RoutePack valide et référencé',async()=>{const pack=JSON.parse(await read('data/routepacks/bonifacio-demo.json')),reg=JSON.parse(await read('data/routes.json'));const report=validateRoutePack(pack);assert.equal(report.valid,true);assert.ok(pack.places.length>=4);assert.ok(reg.routes.some(r=>r.id==='bonifacio-demo'&&r.format==='routepack'))});
...
test('V1.2: Studio expose création, import, validation, partage et export',async()=>{const html=await read('studio.html'),js=await read('js/studio-v1-2.js');for(const token of ['Nouveau parcours','Importer JSON','Validation','Créer le lien','Télécharger JSON','Parcours publiés'])assert.match(html,new RegExp(token));assert.match(js,/validateRoutePack/);assert.match(js,/packShareUrl/);assert.match(js,/routeShareUrl/);assert.match(js,/routepack\.json/)});
