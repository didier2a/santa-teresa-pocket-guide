...
test('V1.8 is a separate entry point extending the production V1.7 engine',()=>{assert.match(entry,/pg16\/bootstrap\/app-rc-entry\.js/);assert.match(entry,/pg17\/bootstrap\/app-enhancer\.js/);assert.match(entry,/app-enhancer\.js\?v=1\.8\.0rc1/);assert.match(enhancer,/__POCKETGUIDE_18__/);});
...
test('V1.8 has isolated state, IndexedDB and service-worker cache',()=>{assert.match(persistence,/pocketguide-18-state-rc1/);assert.match(store,/pocketguide-v18-local/);assert.match(sw,/PG18_CACHE='pocketguide-v18-rc1-local-journal1'/);assert.match(sw,/pocketguide-18\.html/);assert.match(sw,/js\/pg18\/storage\/itinerary-store\.js/);});
