...
test('V1.8 is a separate entry point extending the production V1.7 engine',()=>{assert.match(entry,/pg16\/bootstrap\/app-rc-entry\.js/);assert.match(entry,/pg17\/bootstrap\/app-enhancer\.js/);assert.match(entry,/app-enhancer\.js\?v=1\.8\.0rc6/);assert.match(enhancer,/__POCKETGUIDE_18__/);});
...
test('V1.8 has isolated state, IndexedDB and service-worker cache',()=>{assert.match(persistence,/pocketguide-18-state-rc1/);assert.match(store,/pocketguide-v18-local/);assert.match(sw,/PG18_CACHE='pocketguide-v18-rc1-local-journal6'/);assert.match(sw,/pocketguide-18\.html/);assert.match(sw,/js\/pg18\/storage\/itinerary-store\.js/);});
test('Santa Teresa preview prefers the curated place catalogue over homonymous search results',()=>{assert.match(enhancer,/trip-config\.js\?v=1\.8\.0rc4/);assert.match(enhancer,/V51_PHOTO_MAP/);assert.match(enhancer,/canonicalPlace/);assert.match(enhancer,/photoExact:photo\.exact/);});
test('empty souvenir preview is truthful and disables playback controls',()=>{assert.match(enhancer,/setPreviewControlsAvailable\(false\)/);assert.match(enhancer,/pg18PreviewCount','0 \/ 0'/);assert.match(enhancer,/Aucun souvenir enregistré/);});
test('Voyages tab has a direct V1.8 navigation binding after the asynchronous V1.6 bootstrap',()=>{assert.match(enhancer,/data-tab="library"/);assert.match(enhancer,/openPanel\('library'\)/);});
