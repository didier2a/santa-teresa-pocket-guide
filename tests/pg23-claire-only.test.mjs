...
test('Claire locale est imposée sans flux LiveAvatar',()=>{
  assert.equal(config.defaultMode,'local');assert.equal(config.local.displayName,'Claire');assert.equal(config.local.ready,true);assert.equal(config.local.packVersion,'8');assert.equal(config.live.enabled,false);
...
test('le premier rendu 3D ne dépend pas du téléchargement hors ligne ni de Head Audio',()=>{
  ...
  assert.match(engine,/await this\.prepareHost\(\)/);assert.match(engine,/getContext\('webgl2'/);assert.match(engine,/rendererCanvas:prepared\.canvas/);assert.match(engine,/rendererContext:prepared\.gl/);assert.match(engine,/lipsyncModules:\[\]/);assert.match(engine,/this\.stage='claire-model'/);assert.match(engine,/this\.stage='first-frame'/);assert.match(engine,/readCenterFrame/);assert.match(engine,/Rendu 3D transparent/);assert.match(engine,/1\.35\/deviceRatio/);
...
test('le pack mobile télécharge en parallèle avec délai, reprise et cache',()=>{
  assert.equal(pack.version,'8');assert.equal(pack.cacheName,'pocketguide-local-avatar-v8');assert.ok(pack.assets.length>=19);assert.equal(pack.assets.find(asset=>asset.url.endsWith('/three.core.min.js'))?.bytes,381125);assert.equal(pack.assets.find(asset=>asset.url.endsWith('/talkinghead.mjs'))?.bytes,219842);
...
test('le service worker 2.3.2 installe uniquement des ressources publiées',()=>{
  assert.match(sw,/APP_VERSION='8\.3\.13'/);assert.match(sw,/pocketguide-v23-claire-2-3-2-i/);assert.match(sw,/pocketguide-local-avatar-v8/);assert.match(sw,/endsWith\('\/pocketguide-v23'\)/);assert.doesNotMatch(sw,/docs\/PG23_/);
});
...
test('le diagnostic S22 isole le GPU, le GLB, Three.js puis TalkingHead',()=>{
  assert.match(diagnosticHtml,/Diagnostic autonome de Claire/);assert.match(diagnosticHtml,/claire-diagnostic-v9\.js/);for(const stage of ['WebGL2','Modules Three.js','Téléchargement GLB','Décodage GLB','Rendu Three.js direct','Import TalkingHead','Rendu TalkingHead'])assert.match(diagnosticJs,new RegExp(stage.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));assert.match(diagnosticJs,/EXPECTED_MODEL_HASH/);assert.match(diagnosticJs,/REQUIRED_BONES/);assert.match(diagnosticJs,/lipsyncModules:\[\]/);assert.match(diagnosticJs,/readPixels/);
});
