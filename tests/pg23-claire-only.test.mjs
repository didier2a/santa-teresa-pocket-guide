...
test('Claire locale est imposée sans flux LiveAvatar',()=>{
  assert.equal(config.defaultMode,'local');assert.equal(config.local.displayName,'Claire');assert.equal(config.local.ready,true);assert.equal(config.local.packVersion,'7');assert.equal(config.live.enabled,false);
...
test('le premier rendu 3D ne dépend pas du téléchargement hors ligne ni de Head Audio',()=>{
  ...
  assert.match(engine,/await this\.prepareHost\(\)/);assert.match(engine,/getContext\('webgl2'/);assert.match(engine,/rendererCanvas:prepared\.canvas/);assert.match(engine,/rendererContext:prepared\.gl/);assert.match(engine,/this\.stage='claire-model'/);assert.match(engine,/1\.35\/deviceRatio/);
...
test('le pack mobile télécharge en parallèle avec délai, reprise et cache',()=>{
  assert.equal(pack.version,'7');assert.equal(pack.cacheName,'pocketguide-local-avatar-v7');assert.ok(pack.assets.length>=19);assert.equal(pack.assets.find(asset=>asset.url.endsWith('/three.core.min.js'))?.bytes,381125);assert.equal(pack.assets.find(asset=>asset.url.endsWith('/talkinghead.mjs'))?.bytes,219842);
...
test('le service worker 2.3.2 installe uniquement des ressources publiées',()=>{
  assert.match(sw,/APP_VERSION='8\.3\.12'/);assert.match(sw,/pocketguide-v23-claire-2-3-2-h/);assert.match(sw,/pocketguide-local-avatar-v7/);assert.match(sw,/endsWith\('\/pocketguide-v23'\)/);assert.doesNotMatch(sw,/docs\/PG23_/);
});
