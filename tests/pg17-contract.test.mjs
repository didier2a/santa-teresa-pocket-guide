...
test('V1.7 revisioned imports bypass an already installed stale service worker',()=>{assert.match(html,/app-entry\.js\?v=1\.7\.0rc3/);assert.match(entry,/app-enhancer\.js\?v=1\.7\.0rc3/);});
test('V1.7 reapplies its audiovisual identity after every legacy render event',()=>{assert.match(enhancer,/eventBus\.on\('\*'.*GUIDE AUDIOVISUEL/);});
