...
test('Engine: le route id est lu depuis l URL et filtré',()=>{
...
test('Engine: un RoutePack devient compatible avec le runtime historique',()=>{
  ...
  assert.equal(data.trip.routeId,'demo-route');assert.equal(data.trip.travelers,2);assert.equal(data.places[0].icon,'📍');assert.match(data.places[0].walkingUrl,/travelmode=walking/);assert.equal(data.discover[0].placeId,'p1');
});

test('Engine: le registre sélectionne Santa Teresa par défaut et valide le parcours',async()=>{
  const files={'./data/routes.json':JSON.parse(await read('data/routes.json')),'./data/trip.json':JSON.parse(await read('data/trip.json'))};
  ...
  assert.equal(runtime.route.id,'santa-teresa');assert.equal(runtime.pack.schemaVersion,'1.0');assert.equal(runtime.report.valid,true);assert.ok(runtime.data.days.length>=2);assert.ok(runtime.data.places.length>=5);
});

test('Engine: une route inconnue est refusée avant exécution',async()=>{
  const registry=JSON.parse(await read('data/routes.json'));const fetchImpl=async url=>({ok:url==='./data/routes.json',status:url==='./data/routes.json'?200:404,json:async()=>structuredClone(registry)});
