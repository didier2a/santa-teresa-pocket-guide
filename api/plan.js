...
const ROUTEPACK_SCHEMA={
  ...
  properties:{
    ...
    days:{type:'array',minItems:1,maxItems:10,items:{type:'object',additionalProperties:false,required:['date','label','subtitle','events'],properties:{date:{type:'string'},label:{type:'string'},subtitle:{type:'string'},events:{type:'array',minItems:1,maxItems:12,items:{type:'object',additionalProperties:false,required:['id','time','end','title','type','place','placeId','navigationMode'],properties:{id:{type:'string'},time:{type:'string'},end:{type:'string'},title:{type:'string'},type:{type:'string'},place:{type:'string'},placeId:{type:'string'},navigationMode:{type:'string',enum:['walking','driving','transit']}}}}}}},
    places:{type:'array',minItems:1,maxItems:20,items:{type:'object',additionalProperties:false,required:['id','name','lat','lng','icon','note','description','historyShort','historyLong','repere','arCue','heroImage','sourceLabel','sourceUrl'],properties:{id:{type:'string'},name:{type:'string'},lat:{type:'number',minimum:-90,maximum:90},lng:{type:'number',minimum:-180,maximum:180},icon:{type:'string'},note:{type:'string'},description:{type:'string'},historyShort:{type:'string'},historyLong:{type:'string'},repere:{type:'string'},arCue:{type:'string'},heroImage:{type:'string'},sourceLabel:{type:'string'},sourceUrl:{type:'string'}}}},
...
function systemPrompt(){return `Tu es PocketGuide AI Planner V1.4.6, un planificateur touristique terrain. Transforme la demande en RoutePack V1.0 exécutable. Utilise la recherche web pour vérifier les lieux et informations qui peuvent changer. Priorités: cohérence géographique, journées réalistes, horaires non chevauchants, étapes à pied regroupées, contraintes explicites de l'utilisateur, coordonnées plausibles et sources publiques. N'invente pas d'horaire d'ouverture précis si tu ne peux pas le vérifier. Chaque event.placeId doit référencer exactement un places[].id. Les événements d'une journée doivent être triés et ne pas se chevaucher. Les IDs sont en minuscules ASCII avec tirets. heroImage peut être vide si aucune image fiable n'est disponible. sourceUrl doit être une URL de source publique pertinente. meta.prompt doit reprendre fidèlement la demande utilisateur. Le résultat est un brouillon validable, jamais une garantie de disponibilité. Réponds uniquement selon le schéma fourni.`}
...
export default async function handler(req,res){
  ...
  try{
    ...
    const response=await fetch('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({model,background:true,store:true,max_tool_calls:5,tools:[{type:'web_search'}],reasoning:{effort:'low'},input:[{role:'system',content:[{type:'input_text',text:systemPrompt()}]},{role:'user',content:[{type:'input_text',text:userInput}]}],text:{format:{type:'json_schema',name:'pocketguide_routepack_v1',strict:true,schema:ROUTEPACK_SCHEMA}},metadata:{app:'pocketguide',version:'1.4.6'}})
    });
