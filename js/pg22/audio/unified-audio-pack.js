...
export class UnifiedAudioBus{
  constructor({onLevel}={}){this.onLevel=onLevel||null;this.context=null;this.sources=new WeakMap();this.analysers=[];this.frame=0;this.forcedSilent=false;}
  attach(element){const AudioContextClass=globalThis.AudioContext||globalThis.webkitAudioContext;if(!element||this.sources.has(element)||!AudioContextClass)return false;try{this.context=this.context||new AudioContextClass();const source=this.context.createMediaElementSource(element),analyser=this.context.createAnalyser();analyser.fftSize=256;source.connect(analyser);analyser.connect(this.context.destination);const resume=()=>this.context?.resume?.().catch(()=>{});element.addEventListener('play',resume);element.addEventListener('playing',resume);const channel={element,source,analyser,data:new Uint8Array(analyser.frequencyBinCount),resume};this.sources.set(element,channel);this.analysers.push(channel);this.start();return true;}catch(error){eventBus.emit('pg22.audio.analyser.unavailable',{message:String(error?.message||error)});return false;}}
  start(){if(this.frame)return;const tick=()=>{let level=0;if(!this.forcedSilent)for(const channel of this.analysers){if(channel.element.paused)continue;channel.analyser.getByteTimeDomainData(channel.data);let sum=0;for(const sample of channel.data){const value=(sample-128)/128;sum+=value*value;}level=Math.max(level,Math.sqrt(sum/channel.data.length));}this.onLevel?.(Math.min(1,level*7));this.frame=requestAnimationFrame(tick);};this.frame=requestAnimationFrame(tick);}
...
export class UnifiedVoiceService{
  ...
  install({voiceController,guideAudio,remoteAudio,onLevel}={}){this.guideAudio=guideAudio||this.guideAudio;this.bus=new UnifiedAudioBus({onLevel});this.bus.attach(this.guideAudio);this.bus.attach(remoteAudio);if(voiceController){const stopLocalInput=voiceController.interrupt.bind(voiceController);voiceController.speak=text=>{void this.speak(text);};voiceController.interrupt=()=>{this.interrupt();stopLocalInput();};}return this;}
  ...
  async synthesize(text,{routeId='adhoc',key='narration',signal}={}){
    ...
    const endpoint=this.ttsEndpoint||`${this.apiBase}/v2/speech`,vercel=/\/api\/tts(?:\?|$)/.test(endpoint),payload=vercel?{input:value,voice:this.voice,instructions:'Voix française chaleureuse, cultivée et naturelle de la même accompagnatrice PocketGuide. Diction claire, élégante, jamais théâtrale.'}:{text:value,voice:this.voice,model:this.model,format:'mp3'};
  ...
  async speak(text,{routeId='adhoc',key='narration'}={}){
    ...
    if(!this.guideAudio)return {spoken:false,mode:'cached-no-player'};this.interrupt();this.currentUrl=URL.createObjectURL(clip.blob);this.guideAudio.src=this.currentUrl;this.guideAudio.onended=()=>{this.releaseUrl();eventBus.emit('pg22.audio.ended',{id:clip.id});};await this.guideAudio.play();eventBus.emit('pg22.audio.started',{id:clip.id,routeId:clip.routeId,voice:clip.voice});return {spoken:true,mode:'marin',clip};
  }
  interrupt(){try{this.guideAudio?.pause();if(this.guideAudio)this.guideAudio.currentTime=0;}catch{}this.releaseUrl();this.bus?.silence();try{if(!this.strict)speechSynthesis?.cancel();}catch{}eventBus.emit('pg22.audio.interrupted',{});}
