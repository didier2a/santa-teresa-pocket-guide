...
export class UnifiedAudioBus{
  constructor({onLevel}={}){this.onLevel=onLevel||null;this.context=null;this.sources=new WeakMap();this.streamBindings=new WeakMap();this.analysers=[];this.taps=new Set();this.tapSources=new Map();this.frame=0;this.forcedSilent=false;this.idleFrames=0;this.lastSampleAt=0;}
  ...
  register(channel){this.sources.set(channel.element,channel);this.analysers.push(channel);for(const tap of this.taps)this.connectTap(channel,tap);this.start();eventBus.emit('pg22.audio.analyser.attached',{mode:channel.mode,contextState:this.context?.state||'unknown'});return true;}
  ...
  connectTap(channel,tap){if(!channel?.source||!tap)return false;let sources=this.tapSources.get(tap);if(!sources){sources=new WeakSet();this.tapSources.set(tap,sources);}if(sources.has(channel.source))return false;try{channel.source.connect(tap);sources.add(channel.source);return true;}catch(error){eventBus.emit('pg22.audio.tap.unavailable',{message:String(error?.message||error)});return false;}}
  tap(node){if(!node)return false;this.taps.add(node);for(const channel of this.analysers)this.connectTap(channel,node);eventBus.emit('pg22.audio.tap.attached',{channels:this.analysers.length});return true;}
  untap(node){if(!node||!this.taps.has(node))return false;this.taps.delete(node);for(const channel of this.analysers)try{channel.source?.disconnect?.(node);}catch{}this.tapSources.delete(node);eventBus.emit('pg22.audio.tap.detached',{});return true;}
  ...
  destroy(){if(this.frame)cancelAnimationFrame(this.frame);this.frame=0;for(const tap of [...this.taps])this.untap(tap);this.onLevel?.(0);}
}
