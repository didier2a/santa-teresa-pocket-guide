...
export class UnifiedAudioBus{
  constructor({onLevel}={}){this.onLevel=onLevel||null;this.context=null;this.sources=new WeakMap();this.streamBindings=new WeakMap();this.analysers=[];this.frame=0;this.forcedSilent=false;}
  ...
  start(){if(this.frame||typeof globalThis.requestAnimationFrame!=='function')return;const tick=()=>{let level=0;if(!this.forcedSilent)for(const channel of this.analysers){if(channel.mode==='media-element'&&channel.element.paused)continue;channel.analyser.getByteTimeDomainData(channel.data);let sum=0;for(const sample of channel.data){const value=(sample-128)/128;sum+=value*value;}level=Math.max(level,Math.sqrt(sum/channel.data.length));}this.onLevel?.(Math.min(1,level*7));this.frame=requestAnimationFrame(tick);};this.frame=requestAnimationFrame(tick);}
...
export class UnifiedVoiceService{
  ...
  install({voiceController,guideAudio,remoteAudio,onLevel}={}){this.guideAudio=guideAudio||this.guideAudio;this.bus=new UnifiedAudioBus({onLevel});this.bus.attach(this.guideAudio);this.bus.attachRemote(remoteAudio);if(voiceController){const stopLocalInput=voiceController.interrupt.bind(voiceController);voiceController.speak=text=>{void this.speak(text);};voiceController.interrupt=()=>{this.interrupt();stopLocalInput();};}return this;}
