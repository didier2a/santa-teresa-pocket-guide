...
const planningMediaStates=new Map();

const UNIFIED_PERSONA=`Tu incarnes PocketGuide, une unique accompagnatrice humaine numérique de voyage : chaleureuse, cultivée, éloquente, élégante et attentive. Tu ne te fais jamais passer pour une personne physique. Tu parles en français naturel avec la voix marin. L’utilisateur ne doit jamais percevoir plusieurs IA ou moteurs.
...
function renderThinkingMedia(payload){const host=$('#thinkingMedia');if(!host)return;const place=payload?.place,item=payload?.media||(payload?.items||[]).find(value=>value.placeId===place?.id);if(place?.id)planningMediaStates.set(place.id,{place,item,status:payload?.status||(item?'verified':'unavailable')});const living=/^2\.3/.test($('#companionApp')?.dataset?.pgVersion||''),states=[...planningMediaStates.values()];host.innerHTML=living?states.slice(-6).map(entry=>`<figure data-media-status="${entry.item?'verified':entry.status||'unavailable'}" title="${String(entry.item?.attribution||'').replace(/"/g,'&quot;')}">${entry.item?`<img src="${entry.item.thumbnailUrl||entry.item.url}" alt="${String(entry.item.alt||'Aperçu').replace(/"/g,'&quot;')}">`:'<span aria-hidden="true">◫</span>'}<figcaption>${entry.item?`${entry.item.author} · ${entry.item.license}`:`${entry.place.name} · photo indisponible`}</figcaption></figure>`).join(''):(payload?.items||[]).slice(-4).map(value=>`<figure title="${String(value.attribution||'').replace(/"/g,'&quot;')}"><img src="${value.thumbnailUrl||value.url}" alt="${String(value.alt||'Aperçu').replace(/"/g,'&quot;')}"><figcaption>${value.author} · ${value.license}</figcaption></figure>`).join('');host.hidden=!states.length&&!payload?.items?.length;renderProposalAttributions();}
...
export function installPlanningPipeline(){
  ...
  plannerEngine.proposeReplacement=async request=>{
    ...
    try{
      planningMediaStates.clear();const result=await original({...request,signal});planningStageEngine.set('route');await new Promise(resolve=>setTimeout(resolve,70));
...
export function installAfterV21(){
  ...
  $('#stopCompanion')?.addEventListener('click',()=>{unifiedVoiceService.interrupt();avatarRuntime.interrupt();},{capture:true});const ownsShell=!/^2\.3/.test($('#companionApp')?.dataset?.pgVersion||'');if(ownsShell){document.querySelector('.identity strong').textContent='PocketGuide 2.2';document.title='PocketGuide V2.2 · Votre guide';}
