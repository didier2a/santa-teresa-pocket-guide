...
function gpsFailureMessage(status='error'){
  if(status==='denied')return'La position a été refusée. Autorisez-la dans Chrome, puis touchez « Autoriser ma position » pour réessayer.';
  if(status==='unavailable')return'Ce navigateur ne fournit pas la géolocalisation. La carte et la simulation restent disponibles.';
  return'Je n’ai pas reçu de position fiable. Vérifiez la localisation du téléphone, puis réessayez.';
}
function requestLocation(){setText('#locationStatus','La position n’est demandée qu’après votre accord.');const button=$('#allowPosition');if(button)button.disabled=false;showDialog($('#locationDialog'));}
...
function renderHero(){
  ...
  const gps=state.perception.gps;setText('#gpsSense',gps==='ready'?(state.location.accuracy?`Position ±${Math.round(state.location.accuracy)} m`:'Position active'):gps==='starting'?'Recherche GPS…':gps==='denied'?'Position refusée':gps==='degraded'?'Position imprécise':gps==='error'?'GPS indisponible':gps==='unavailable'?'GPS non pris en charge':'Position en attente');
...
function installEvents(){
  ...
  $$('[data-journey-mode]').forEach(button=>button.addEventListener('click',()=>showJourneyMode(button.dataset.journeyMode)));$('#startGps').addEventListener('click',requestLocation);$('#allowPosition').addEventListener('click',async()=>{const button=$('#allowPosition');if(button)button.disabled=true;setText('#locationStatus','Je recherche une position GPS fiable…');setPresence({value:'connecting',label:'Je vous situe…'});const orientationPermission=platformAdapter.requestOrientationPermission(),started=await perceptionEngine.startLocation({waitForResult:true}),orientationGranted=await orientationPermission;if(started){if(orientationGranted)await perceptionEngine.startOrientation({requestPermission:false});setText('#locationStatus','Position active. Le guidage étape par étape peut commencer.');closeDialog($('#locationDialog'));setPresence({value:'ready',label:'Je vous guide',connected:realtimeCompanion.connected});}else{const status=pocketGuideState.select('perception.gps'),message=gpsFailureMessage(status);setText('#locationStatus',message);setText('#guideActionStatus',message);setPresence({value:'degraded',label:'Position indisponible'});toast(message,6500);}if(button)button.disabled=false;renderHero();});$('#positionLater').addEventListener('click',()=>closeDialog($('#locationDialog')));
  $('#simulateJourney').addEventListener('click',()=>openPreview());$('#createByVoice').addEventListener('click',()=>{setView('companion');void companionOrchestrator.toggleListening();toast('Dites-moi la destination, la durée, le rythme et vos envies.');});$('#createByText').addEventListener('click',()=>{const dialog=$('#plannerDialog');if(dialog)dialog.dataset.plannerMode='create';showDialog(dialog);});
  $('#plannerForm').addEventListener('submit',async event=>{event.preventDefault();const prompt=$('#plannerPrompt').value.trim();if(prompt.length<8){setText('#plannerStatus','Décrivez un peu davantage votre excursion.');return;}const dialog=$('#plannerDialog'),pack=pocketGuideState.select('route.pack'),editMode=dialog?.dataset.plannerMode==='edit'&&Boolean(pack?.id),requestPrompt=editMode?buildRouteRevisionPrompt(prompt,pack):prompt,placeTarget=editMode?Math.max(Number($('#plannerPlaces').value)||5,(pack.places||[]).length):Number($('#plannerPlaces').value)||5;adaptiveMomentEngine.setPlanning(true);setText('#plannerStatus','Je vérifie les lieux et je construis un parcours cohérent…');try{const result=await plannerEngine.proposeReplacement({prompt:requestPrompt,destination:editMode?'':$('#plannerDestination').value,maxPlaces:Math.max(3,Math.min(10,placeTarget))});pendingPlanPack=result.plan.pack;renderProposalSummary(pendingPlanPack);setText('#plannerStatus',`« ${result.plan.pack.title} » est prêt. Je demande votre confirmation.`);closeDialog(dialog);setText('#proposalText',result.proposal.summary);showDialog($('#proposalDialog'));}catch(error){setText('#plannerStatus',`Je n’ai pas pu préparer cette excursion : ${error.message||error}`);}finally{adaptiveMomentEngine.setPlanning(false);}});
  ...
  eventBus.on('gps.updated',()=>{setText('#locationStatus','Position active. Le guidage étape par étape peut commencer.');if($('#locationDialog')?.open)closeDialog($('#locationDialog'));setPresence({value:'ready',label:'Je vous guide',connected:realtimeCompanion.connected});walkingGuidanceEngine.processPosition();realtimeCompanion.sendContext('position mesurée');renderHero();adaptiveMomentEngine.sync();});for(const type of ['gps.denied','gps.error','gps.unavailable'])eventBus.on(type,()=>{const status=pocketGuideState.select('perception.gps'),message=gpsFailureMessage(status);setText('#guideActionStatus',message);setText('#locationStatus',message);setPresence({value:'degraded',label:'Position indisponible'});renderHero();if(!$('#locationDialog')?.open)toast(message,6500);});eventBus.on('heading.updated',()=>{realtimeCompanion.sendContext('orientation');renderHero();});eventBus.on('guidance.snapshot',payload=>{renderHero();adaptiveMomentEngine.snapshot=payload;adaptiveMomentEngine.sync();});eventBus.on('guidance.cue',payload=>{appendTurn('companion',payload.text,{source:'guidance'});if(realtimeCompanion.connected)realtimeCompanion.announce(payload.text);else if(!session.quietMode)voiceController.speak(payload.text);});
