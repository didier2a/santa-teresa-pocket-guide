...
export class GuideCommandRouter{
  ...
  async execute(intent,text,{source='guide'}={}){
    switch(intent.type){
      ...
      case'show_route_content':{await this.actions.execute('ui.open_map',{}, {source:'pg233-command'});const execution=await this.actions.execute('pg23.present_route',{speak:false},{source:'pg233-command'});return outcome(intent.type,'J’affiche la carte, les étapes, les fiches et les photographies disponibles du parcours.',{execution});}
      case'show_place':{
