...
import {registerV2Actions} from '../core/v2-actions.js';
import {loadCompanionSession,saveCompanionSession,resetSensorsForRealSession} from '../core/session-store.js';
import {companionOrchestrator} from '../companion/companion-orchestrator.js';
import {realtimeCompanion} from '../companion/realtime-companion.js';
...
const session=loadCompanionSession()||{currentItineraryId:null,quietMode:false,voiceEnabled:true};
const voiceRecorder=new LocalVoiceNoteRecorder();
let map=null,routeLayer=null,positionMarker=null,toastTimer=null,showArchives=false,pendingPhoto=null,pendingPhotoUrl=null,pendingVoiceNote=null,previewItineraryId=null,previewVoice=true,arRenderTimer=null;
const journalUrls=[];
...
function enrichKnownMedia(){
  ...
  if(changed)pocketGuideState.patch({route:{pack:{...pack,places}}},{source:'pg2-media',event:'route.media.enriched'});return changed;
}

function setPresence({value='ready',label='Je suis avec vous',connected=false,listening=false,message=''}={}){
  app.dataset.presence=value;setText('#presenceLabel',label);setText('#connectionLabel',value==='degraded'?'Mode essentiel':connected?'Avec vous en direct':'Avec vous');
...
function setView(view){
  const target=['companion','journey','memories'].includes(view)?view:'companion';if(pocketGuideState.select('ui.panel')!==target)pocketGuideState.patch({ui:{panel:target}},{source:'pg2-ui',event:'ui.panel.changed'});
...
async function openPreview(id=currentItineraryId()){
  ...
  previewItineraryId=id;photoPreviewEngine.load({itinerary,media,mode:MODES.PREPARATORY});setText('#previewTitle',itinerary.label||itinerary.title);showDialog($('#previewDialog'));
}
...
async function openAR(){
  ...
  pocketGuideState.patch({ui:{ar:true,arRequested:false,panel:'companion'}},{source:'pg2-ar',event:'ar.opened'});$('#arLayer').hidden=false;setView('companion');setPresence({value:'ready',label:'Je regarde avec vous',connected:realtimeCompanion.connected});renderARLabels();arRenderTimer=setInterval(renderARLabels,300);
}
...
function closeAR(){clearInterval(arRenderTimer);arRenderTimer=null;perceptionEngine.closeCamera($('#companionCamera'));pocketGuideState.patch({ui:{ar:false,arRequested:false}},{source:'pg2-ar',event:'ar.closed'});$('#arLayer').hidden=true;setPresence({value:'ready',label:'Je suis avec vous',connected:realtimeCompanion.connected});}
...
function installEvents(){
  companionOrchestrator.install({remoteAudio:$('#remoteAudio')});companionOrchestrator.setVoiceOutput(!session.quietMode);companionOrchestrator.onTurn=(role,text,meta)=>appendTurn(role,text,meta);companionOrchestrator.onStatus=setPresence;companionOrchestrator.onProposal=proposal=>{if(proposal){setText('#proposalText',proposal.summary||'Je vous propose un changement.');showDialog($('#proposalDialog'));}else closeDialog($('#proposalDialog'));};
  ...
  $('#startTogether').addEventListener('click',async()=>{closeDialog($('#welcomeDialog'));session.quietMode=false;companionOrchestrator.setVoiceOutput(true);saveCompanionSession({...session,currentItineraryId:currentItineraryId()});setPresence({value:'connecting',label:'Je vous rejoins…'});await Promise.allSettled([perceptionEngine.startLocation(),companionOrchestrator.startVoice()]);renderHero();});
  ...
  $$('[data-journey-mode]').forEach(button=>button.addEventListener('click',()=>showJourneyMode(button.dataset.journeyMode)));$('#startGps').addEventListener('click',()=>perceptionEngine.startLocation());
  $('#simulateJourney').addEventListener('click',()=>openPreview());$('#createByVoice').addEventListener('click',()=>{setView('companion');void companionOrchestrator.toggleListening();toast('Dites-moi la destination, la durée et vos envies.');});$('#createByText').addEventListener('click',()=>showDialog($('#plannerDialog')));
  $('#plannerForm').addEventListener('submit',async event=>{event.preventDefault();const prompt=$('#plannerPrompt').value.trim();if(prompt.length<8){setText('#plannerStatus','Décrivez un peu davantage votre excursion.');return;}setText('#plannerStatus','Je vérifie les lieux et je construis un parcours cohérent…');try{const result=await plannerEngine.proposeReplacement({prompt,destination:$('#plannerDestination').value,maxPlaces:Number($('#plannerPlaces').value)||5});setText('#plannerStatus',`« ${result.plan.pack.title} » est prêt. Je demande votre confirmation.`);closeDialog($('#plannerDialog'));setText('#proposalText',result.proposal.summary);showDialog($('#proposalDialog'));}catch(error){setText('#plannerStatus',`Je n’ai pas pu préparer cette excursion : ${error.message||error}`);}});
  ...
  eventBus.on('ui.panel.changed',payload=>setView(payload?.after?.ui?.panel||pocketGuideState.select('ui.panel')));eventBus.on('ui.preview.requested',()=>openPreview());eventBus.on('ui.vision.requested',()=>$('#photoInput').click());eventBus.on('ui.journal.requested',()=>openJournal());eventBus.on('ar.requested',()=>{toast('Touchez deux fois « Regarde avec moi » pour ouvrir la vue AR avec votre accord.',4500);});
  eventBus.on('gps.updated',()=>{walkingGuidanceEngine.processPosition();realtimeCompanion.sendContext('position mesurée');renderHero();});eventBus.on('heading.updated',()=>{realtimeCompanion.sendContext('orientation');renderHero();});eventBus.on('guidance.snapshot',renderHero);eventBus.on('guidance.cue',payload=>{appendTurn('companion',payload.text,{source:'guidance'});if(realtimeCompanion.connected)realtimeCompanion.announce(payload.text);else if(!session.quietMode)voiceController.speak(payload.text);});
...
async function boot(){
  ...
  await loadInitialRoute();enrichKnownMedia();pocketGuideState.patch({version:'2.0.0-rc1',boot:{status:'ready',startedAt:new Date().toISOString()},session:{id:`pg2_${Date.now().toString(36)}`,startedAt:new Date().toISOString(),simulation:false},ui:{panel:'companion',journeyMode:'timeline'}},{source:'pg2-bootstrap',event:'app.ready'});
  installEvents();walkingGuidanceEngine.start();itineraryManager.start();await itineraryManager.saveCurrent('pg2-initial');session.currentItineraryId=currentItineraryId();saveCompanionSession(session);
  appendTurn('companion',`Je suis avec vous. Votre voyage « ${pocketGuideState.select('route.title')} » est prêt. Parlez-moi naturellement ou explorez à votre rythme.`,{source:'local'});renderHero();renderTimeline();
  ...
  globalThis.__POCKETGUIDE_V2__={state:pocketGuideState,bus:eventBus,actions:actionRegistry,companion:companionOrchestrator,realtime:realtimeCompanion,planner:plannerEngine,perception:perceptionEngine,guidance:walkingGuidanceEngine,simulator:walkingSimulator,itineraries:itineraryManager,preview:photoPreviewEngine,openAR,openPreview,openJournal,render:renderHero};
}

boot().catch(error=>{console.error(error);setPresence({value:'error',label:'Je n’ai pas pu ouvrir le voyage',message:error.message||String(error)});appendTurn('companion',`Erreur de démarrage : ${error.message||error}`,{source:'error'});toast('PocketGuide V2 n’a pas pu terminer son démarrage.',5000);});
