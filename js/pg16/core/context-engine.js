...
export class HumanContextEngine {
  ...
  build(){const s=this.state.get(),route=s.route||{},location=s.location||{},conversation=s.conversation||{},named=routeObjects(route);return {
    ...
    perception:{...s.perception},device:{online:Boolean(s.device?.online),platform:s.device?.platform||'web',standalone:Boolean(s.device?.standalone),battery:finite(s.device?.battery)},connectivity:{...(s.connectivity||{})},
    ...
    preferences:{session:{...(s.preferences?.session||{})},persistent:{...(s.preferences?.persistent||{})},memory:{working:{...(s.memory?.working||{})},session:{...(s.memory?.session||{})}},
    ...
    ui:{panel:s.ui?.panel||'guide',ar:Boolean(s.ui?.ar)},capabilities:{canLocate:s.perception?.gps==='ready',canOrient:s.perception?.orientation==='ready',canSee:s.perception?.camera==='ready',canHear:s.perception?.microphone==='ready',online:Boolean(s.device?.online)}
  };}
