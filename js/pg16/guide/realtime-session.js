...
export class RealtimeSession {
  ...
  sendContext(reason='context'){
    if(!this.connected)return false;const context=humanContextEngine.build();return this.send({type:'session.update',session:{type:'realtime',instructions:`${instructions()}\nContexte PocketGuide (${reason}): ${JSON.stringify(context)}`,tools:TOOLS}});
  }
  ...
  interrupt(){if(this.connected)this.send({type:'response.cancel'});this.status('idle','Guide');}
  ...
  async executeTool(name,args={}){
    ...
    if(name==='undo_last_change'){
      const reply=await import('../core/transaction-manager.js').then(({transactionManager})=>transactionManager.undo());return {ok:Boolean(reply),transaction:reply?.name||null};
    }
  ...
  async handleTool(name,callId,argsText){
    ...
    let result;try{result=await this.executeTool(name,parseArgs(argsText));}catch(error){result={error:String(error?.message||error)};}
    this.send({type:'conversation.item.create',item:{type:'function_call_output',call_id:callId,output:JSON.stringify(result)}});this.sendContext('tool-result');this.send({type:'response.create'});return result;
  }

  handleEvent(evt){
    if(evt.type==='response.created'){this.status('speaking','Le guide prépare sa réponse…');return;}
    ...
    if(evt.type==='response.done'){this.status('idle','Guide IA prêt');return;}
  ...
  async connect({remoteAudio}={}){
    ...
    try{
      const cfg=await loadConfig();if(!cfg.apiBase)throw new Error('Pont IA non configuré');
      if(typeof RTCPeerConnection!=='function')throw new Error('WebRTC indisponible');
      this.status('connecting','Connexion au guide…');
      const pc=new RTCPeerConnection();this.pc=pc;
      pc.ontrack=event=>{if(this.remoteAudio){this.remoteAudio.srcObject=event.streams[0];this.remoteAudio.play().catch(()=>{});}};
      pc.onconnectionstatechange=()=>{if(['failed','disconnected','closed'].includes(pc.connectionState)){this.connected=false;this.status('idle','Guide local');eventBus.emit('realtime.disconnected',{state:pc.connectionState});}};
      const stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});this.stream=stream;stream.getTracks().forEach(track=>pc.addTrack(track,stream));pocketGuideState.patch({perception:{microphone:'ready'}},{source:'realtime',event:'microphone.ready'});
      ...
      const response=await fetch(`${cfg.apiBase}/v2/realtime/call?voice=${encodeURIComponent(cfg.voice||'marin')}`,{method:'POST',headers:{'Content-Type':'application/sdp'},body:offer.sdp});
      const answer=await response.text();if(!response.ok)throw new Error(answer||`Realtime ${response.status}`);await pc.setRemoteDescription({type:'answer',sdp:answer});return true;
  ...
  disconnect(){try{this.dc?.close()}catch{}try{this.pc?.close()}catch{}try{this.stream?.getTracks().forEach(t=>t.stop())}catch{}this.dc=null;this.pc=null;this.stream=null;this.connected=false;this.connecting=false;this.status('idle','Guide local');}
}
