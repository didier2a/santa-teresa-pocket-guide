...
export class ProactiveEngine {
  ...
  onGps(){const state=pocketGuideState.get();if(state.perception?.gps!=='ready')return;const accuracy=Number(state.location?.accuracy);if(Number.isFinite(accuracy)&&accuracy>this.options.gpsAccuracyBad){this.suggest('gps_degraded','Le GPS est devenu imprécis. Je préfère ne pas vous donner une direction exacte pour le moment.',{accuracy});return;}
    const nearest=geoAREngine.nearest(1)[0];if(!nearest)return;const meters=Math.round(nearest.distanceKm*1000);if(meters<=this.options.poiRadiusMeters){const previous=this.poiAt.get(nearest.place.id)||0;if(Date.now()-previous>=this.options.poiCooldownMs){this.poiAt.set(nearest.place.id,Date.now());this.suggest('near_place',`Nous approchons de ${nearest.place.name}. Vous voulez que je vous raconte ce lieu ?`,{placeId:nearest.place.id,distanceMeters:meters});}}
  }
