...
const DEFAULTS={poiRadiusMeters:110,globalCooldownMs:75_000,poiCooldownMs:12*60_000,offRouteMeters:450,gpsAccuracyBad:80,timePressureMarginMinutes:5};
export class ProactiveEngine {
  ...
  onTimeAvailable(payload={}){const available=Number(payload.availableMinutes),remaining=Number(pocketGuideState.select('route.remainingMinutes'));if(!Number.isFinite(available)||available<=0||!Number.isFinite(remaining)||remaining<=0)return false;if(remaining<=available+this.options.timePressureMarginMinutes)return false;return this.suggest('time_pressure',`Il reste environ ${Math.round(remaining)} minutes de parcours, mais seulement ${Math.round(available)} minutes disponibles. Je peux raccourcir la balade en gardant les incontournables.`,{availableMinutes:available,remainingMinutes:remaining,shortageMinutes:Math.max(0,Math.round(remaining-available))});}
  ...
  start(){if(this.started)return;this.started=true;this.unsubs.push(eventBus.on('gps.updated',()=>this.onGps()),eventBus.on('time.available.updated',payload=>this.onTimeAvailable(payload)),eventBus.on('network.offline',()=>this.onNetwork(false)),eventBus.on('network.online',()=>this.onNetwork(true)),eventBus.on('route.completed',()=>this.onRouteCompleted()));}
