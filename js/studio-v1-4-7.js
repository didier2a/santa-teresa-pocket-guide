...
let current=null,lastReport=null,apiBase='',recorder=null,stream=null,chunks=[],recordTimer=null,speechRec=null;

function esc(s=''){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c]))}
function setStatus(text,type=''){const el=$('#status');if(!el)return;el.className=type;el.innerHTML=text}
function setVoice(text,type=''){const el=$('#voiceStatus');if(!el)return;el.className=type;el.textContent=text}
...
async function pollPlan(taskId){
  ...
  while(Date.now()-started<240000){
    ...
    if(r.status===202){await sleep(Math.min(5000,1800+attempt*180));continue}if(!r.ok)throw new Error(p.error||`Erreur ${r.status}`);if(p.pack){await showPack(p.pack);return}await sleep(2500)
  }
  throw new Error('La génération prend trop de temps. Réessayez dans quelques instants.');
}

async function plan(){
  const prompt=$('#prompt').value.trim();if(prompt.length<8){setStatus('Décrivez un peu plus votre voyage.','bad');return}if(!apiBase){setStatus('Le backend AI Planner n’est pas encore relié.','bad');return}
...
// Bloc vocal repris de la V1.4.6 validée sur Chrome Android / Galaxy S22.
...
async function transcribeBlob(blob){if(!apiBase)throw new Error('Backend de transcription non relié');setVoice('Transcription OpenAI en cours…','work');const audio=await blobToBase64(blob);const r=await fetch(endpoint('/api/transcribe'),{method:'POST',headers:{'Content-Type':'text/plain;charset=UTF-8'},body:JSON.stringify({audio,mimeType:blob.type||'audio/webm'})});const payload=await r.json().catch(()=>({}));if(!r.ok)throw new Error(payload.error||`Erreur ${r.status}`);return String(payload.text||'').trim()}
...
async function startRecorder(){stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true}});chunks=[];const mimeType=bestMime();recorder=new MediaRecorder(stream,mimeType?{mimeType}:undefined);recorder.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data)};recorder.onstop=async()=>{clearTimeout(recordTimer);$('#mic').classList.remove('recording');$('#mic').textContent='🎙️ Parler';stream?.getTracks().forEach(t=>t.stop());try{const blob=new Blob(chunks,{type:recorder.mimeType||'audio/webm'});if(blob.size<500)throw new Error('Enregistrement trop court');appendTranscript(await transcribeBlob(blob))}catch(err){setVoice(err.message||'Transcription impossible','bad')}};recorder.start(250);$('#mic').classList.add('recording');$('#mic').textContent='■ Arrêter';setVoice('● Écoute en cours… Parlez naturellement.','recording');recordTimer=setTimeout(stopRecorder,90000)}
function browserSpeech(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR)throw new Error('Reconnaissance vocale indisponible dans ce navigateur.');if(speechRec){try{speechRec.stop()}catch{}return}const rec=new SR();speechRec=rec;rec.lang='fr-FR';rec.interimResults=true;rec.continuous=false;let bestFinal='',bestInterim='',gotSpeech=false,hadError=false;rec.onstart=()=>{$('#mic').classList.add('recording');$('#mic').textContent='■ Arrêter';setVoice('● Je vous écoute… la phrase sera insérée une seule fois à la fin.','recording')};rec.onspeechstart=()=>{gotSpeech=true;setVoice('● Voix détectée… continuez à parler.','recording')};rec.onresult=e=>{let finalCandidate='',interimCandidate='';for(let i=0;i<e.results.length;i++){const t=String(e.results[i][0]?.transcript||'').trim();if(!t)continue;gotSpeech=true;if(e.results[i].isFinal)finalCandidate=[finalCandidate,t].filter(Boolean).join(' ');else interimCandidate=t}if(finalCandidate)bestFinal=finalCandidate;if(interimCandidate)bestInterim=interimCandidate;setVoice(`« ${dedupeTranscript(bestFinal||bestInterim)} »`,'work')};rec.onerror=e=>{hadError=true;const msg=e.error==='not-allowed'?'Autorisation micro refusée dans Chrome.':e.error==='no-speech'?'Aucune parole détectée.':e.error==='network'?'Service de dictée Chrome indisponible.':'Dictée interrompue : '+e.error;setVoice(msg,'bad')};rec.onend=()=>{speechRec=null;$('#mic').classList.remove('recording');$('#mic').textContent='🎙️ Parler';const transcript=dedupeTranscript(bestFinal||bestInterim);if(transcript)appendTranscript(transcript);else if(!gotSpeech&&!hadError)setVoice('Aucun texte reconnu. Appuyez sur Parler puis dictez votre demande.','bad')};rec.start()}
async function toggleVoice(){if(speechRec){try{speechRec.stop()}catch{}return}if(recorder?.state==='recording'){await stopRecorder();return}try{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(SR){browserSpeech();return}if(apiBase&&navigator.mediaDevices?.getUserMedia&&window.MediaRecorder){await startRecorder();return}throw new Error('Aucun moteur de dictée compatible n’est disponible dans ce navigateur.')}catch(err){setVoice(err.message||'Micro indisponible','bad')}}

const mic=$('#mic');if(mic)mic.addEventListener('click',toggleVoice);
