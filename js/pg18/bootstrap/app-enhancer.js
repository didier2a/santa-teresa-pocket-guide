...
async function openPreview(id=currentId(),mode=$('#pg18PreviewMode')?.value||MODES.PREPARATORY){
  ...
  await itineraryManager.flush().catch(()=>null);let itinerary=await itineraryStore.getItinerary(id);if(!itinerary)throw new Error('Itinéraire local introuvable.');itinerary=await ensurePreviewMedia(itinerary);const media=await itineraryStore.listMedia(id);previewItineraryId=id;$('#pg18PreviewMode').value=mode;const scenes=photoPreviewEngine.load({itinerary,media,mode});setText('#pg18PreviewTitle',itinerary.label||itinerary.title);showDialog($('#pg18PreviewDialog'));if(!scenes.length){setPreviewControlsAvailable(false);setText('#pg18PreviewProvenance',mode===MODES.SOUVENIR?'Photos personnelles':'RoutePack');setText('#pg18PreviewPlace','Aucune image dans cette version');setText('#pg18PreviewStory',mode===MODES.SOUVENIR?'Prenez des photos pendant la visite ou choisissez la version préparatoire.':'Aucune scène disponible.');setText('#pg18PreviewLeg','Aucun souvenir enregistré');setText('#pg18PreviewCount','0 / 0');const bar=$('#pg18PreviewProgress');if(bar)bar.style.width='0%';const preview=$('#pg18PreviewMedia');if(preview)preview.style.setProperty('--preview','linear-gradient(135deg,#145966,#0a2429)');}else renderPreviewScene(scenes[0]);
}
