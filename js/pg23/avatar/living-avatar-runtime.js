...
export class LipSyncLabRuntime{
  ...
  install({root,portrait,mouth,readout={}}={}){this.root=root;this.portrait=portrait;this.mouth=mouth;this.readout=readout;this.fit();this.renderMeta({source:'silencieux',state:'ready',mode:'lab',version:'2.3.1'});return this;}
  ...
  async runMarin(text='Bonjour, je suis votre guide audiovisuelle.'){
    if(this.running)return{ok:false,code:'busy',label:'Test déjà en cours'};this.running=true;this.changes=0;this.last='neutral';if(this.root)this.root.dataset.avatarState='speaking';this.renderMeta({source:'marin',state:'speaking',mode:'lab',version:'2.3.1',audio:'Démarrage…'});if(this.readout.status)this.readout.status.textContent='Voix marin et visage principal en cours de contrôle…';this.bus.emit('pg23.lab.started',{label:'Voix marin'});
