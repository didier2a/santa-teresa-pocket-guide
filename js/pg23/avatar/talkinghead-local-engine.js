...
import {ClaireThreeStage} from './claire-three-stage.js';
...
export class TalkingHeadLocalEngine{
  ...
  async activate(){
    ...
    try{
      ...
      this.stage='first-frame';await nextFrame();if(this.head.isRunning)this.head.animate(1000/modelFPS);let fit=this.threeStage.fit(this.head.armature,this.config.cameraView||'upper');this.frameDiagnostic={...this.threeStage.sample(),fit,renderer:'three-external'};if(!this.frameDiagnostic.ok){fit=this.threeStage.fit(this.head.armature,'full');this.frameDiagnostic={...this.threeStage.sample(),fit,fallbackView:'full',renderer:'three-external'};}if(!this.frameDiagnostic.ok)throw new Error(`Rendu 3D transparent (${this.frameDiagnostic.render?.calls||0} appels, ${this.frameDiagnostic.render?.triangles||0} triangles)`);
      this.stage='active';this.active=true;this.startedAt=Date.now();this.threeStage.start(this.head);this.host.hidden=false;this.host.style.visibility='';this.host.removeAttribute('aria-hidden');this.portrait?.setAttribute?.('aria-hidden','true');this.bus.emit('pg23.avatar.engine.active',{engine:this.id,identity:'Claire'});void this.installAudio(session);return{active:true,audioPending:true};
