...
export class TalkingHeadLocalEngine{
  constructor({bus=eventBus}={}){this.bus=bus;this.id='local';this.host=null;this.portrait=null;this.audioBus=null;this.config=null;this.head=null;this.headAudio=null;this.active=false;this.audioReady=false;this.audioError='';this.lastError='';this.stage='idle';this.startedAt=0;this.session=0;}
  ...
  diagnostic(){return{id:this.id,identity:'Claire',active:this.active,ready:Boolean(this.config?.ready),stage:this.stage,audioReady:this.audioReady,capabilities:this.capabilities(),startedAt:this.startedAt||null,error:this.lastError||null,audioError:this.audioError||null};}
