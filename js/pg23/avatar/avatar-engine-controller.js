...
export class AvatarEngineController{
  ...
  async apply(){
    ...
    if(this.activeId!==target){
      ...
      catch(error){const message=String(error?.message||error);await engine.destroy?.();const fallback=this.engines.get('portrait');await fallback.activate();this.activeId='portrait';this.lastDecision={mode:'portrait',reason:'local-runtime-failed',message};if(this.nodes.root)this.nodes.root.dataset.avatarError=message;if(this.nodes.portrait){this.nodes.portrait.querySelector('strong')?.replaceChildren('Claire doit être relancée');this.nodes.portrait.querySelector('small')?.replaceChildren(message);}this.bus.emit('pg23.avatar.engine.failed',{engine:target,message});}
    }
  ...
  async retry(){if(this.nodes.retry)this.nodes.retry.hidden=true;if(this.nodes.portrait){this.nodes.portrait.querySelector('strong')?.replaceChildren('Claire se prépare');this.nodes.portrait.querySelector('small')?.replaceChildren('Avatar 3D local · nouvelle tentative');}this.lastDecision={mode:'local',reason:'local-loading'};this.renderStatus('preparing');const result=await this.ensureLocalPack({onProgress:progress=>{if(this.nodes.status)this.nodes.status.textContent=`Nouvelle tentative · ${progress.index}/${progress.total}`;}});if(result.installed&&this.activeId!=='local')await this.apply();return result;}
