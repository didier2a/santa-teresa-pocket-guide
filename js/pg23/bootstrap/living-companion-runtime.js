...
function installActions(){
  if(!actionRegistry.has('pg23.present_route'))actionRegistry.register('pg23.present_route',{description:'Présenter audiovisuellement chaque étape du RoutePack réel.',riskLevel:'safe',confirmation:'none',handler:async(args={},context={})=>{const pack=routePack();if(!pack?.id)throw new Error('Aucun itinéraire actif à présenter.');const result=await presentationDirector.present(pack,{source:context.source||'action',speak:Boolean(args.speak)});if(!result.ok){const error=new Error(result.reason==='interrupted'?'La présentation a été interrompue.':'La présentation du parcours n’a pas pu démarrer.');error.code=`PRESENTATION_${String(result.reason||'failed').toUpperCase()}`;throw error;}return{...result,routeId:pack.id,places:pack.places?.length||0};}});
}
...
function installSceneBridges(){
  ...
  eventBus.on('pg23.presentation.started',payload=>{livingPresenceMachine.transition('presenting',{source:'presentation',reason:'started',label:'Je vous montre le parcours',portrait:'guide'});if(payload?.source!=='pg233-route-content')setTimeout(()=>$('#livingFlow')?.scrollIntoView?.({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'}),80);});
