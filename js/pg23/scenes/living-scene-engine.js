...
const safeUrl=value=>{try{const url=new URL(String(value||''),globalThis.location?.href||'https://pocketguide.local/');return ['https:','http:','blob:','data:'].includes(url.protocol)?url.href:'';}catch{return'';}};
...
export class LivingSceneEngine{
  ...
  clearEphemeral(){const keep=this.persistent();this.items=keep;this.ids=new Map(keep.map(item=>[item.id,item]));if(this.host)for(const node of this.host.querySelectorAll('[data-scene-id]'))if(!this.ids.has(node.dataset.sceneId))node.remove();this.renderCount();return keep;}
}
